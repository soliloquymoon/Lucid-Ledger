function dream_draw_item(xx, yy, dream, display_price, scale = 1){
	...
	if (dream.unlocks[0]) {
		...
		draw_sprite(spr_trend_indicator, ord(string_char_at(tag_strs[0], 1)) mod 4, xx + 160 * scale, yy + 63 * scale)
	} else {
		draw_sprite(spr_tag_back, 4, xx + 160 * scale, yy + 63 * scale)
	}
	set_tooltip(xx + 160 * scale, yy + 63 * scale, 58 * scale, 18 * scale, tag_strs[0])
	if (dream.unlocks[1]) {
		draw_sprite(spr_tag_back, 3 * (dream.tags[1].tag_name != ""), xx + 254 * scale, yy + 63 * scale)
		if (dream.tags[1].tag_name != "") draw_sprite(spr_trend_indicator, ord(string_char_at(tag_strs[1], 1)) mod 4, xx + 254 * scale, yy + 63 * scale)
	} else {
		draw_sprite(spr_tag_back, 4, xx + 254 * scale, yy + 63 * scale)
	}
	set_tooltip(xx + 254 * scale, yy + 63 * scale, 58 * scale, 18 * scale, tag_strs[1])
	if (dream.unlocks[2]) {
		draw_sprite(spr_tag_back, 3 * (dream.tags[2].tag_name != ""), xx + 160 * scale, yy + 88 * scale)
		if (dream.tags[2].tag_name != "") draw_sprite(spr_trend_indicator, ord(string_char_at(tag_strs[2], 1)) mod 4, xx + 160 * scale, yy + 88 * scale)
	} else {
		draw_sprite(spr_tag_back, 4, xx + 160 * scale, yy + 88 * scale)
	}
	set_tooltip(xx + 160 * scale, yy + 88 * scale, 58 * scale, 18 * scale, tag_strs[2])
	if (dream.unlocks[3]) {
		draw_sprite(spr_tag_back, 3 * (dream.tags[3].tag_name != ""), xx + 254 * scale, yy + 88 * scale)
		if (dream.tags[3].tag_name != "") draw_sprite(spr_trend_indicator, ord(string_char_at(tag_strs[3], 1)) mod 4, xx + 254 * scale, yy + 88 * scale)
	} else {
